...
if ($role === 'dosen') {
    ...
    foreach ($documents as &$document) {
        ...
        $document['perlu_review_saya'] = $document['status'] === 'menunggu_review' && $myDecision !== 'disetujui' && p2_can_review($conn, (int)$document['bimbingan_id'], 'bab', (int)$document['id'], $userId);
    }
    unset($document);
}
...
$pendingCount = count(array_filter($documents, function ($item) use ($role) {
    return $role === 'mahasiswa' ? $item['status'] === 'direvisi' : !empty($item['perlu_review_saya']);
}));
$documentsByStudent = [];
$pendingByStudent = [];
if ($role === 'dosen') {
