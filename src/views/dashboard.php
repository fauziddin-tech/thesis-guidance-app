...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'mark_seminar_done' && in_array($role, ['dosen', 'admin'], true)) {
        try {
        ...
        $redirectDashboard($role === 'admin' ? 'daftar-bimbingan' : 'mahasiswa-bimbingan');
    }

    if ($action === 'unmark_seminar' && $role === 'admin') {
        try {
            proposal_unmark_seminar($conn, (int)($_POST['bimbingan_id'] ?? 0));
