...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'upload_chapter' && $role === 'mahasiswa') {
        ...
        $uploadState = $guidance ? ($postedChapter === P2_FORMAT_TARGET ? p2_format_upload_state($conn, $guidanceId) : chapter_upload_state($conn, $guidanceId)) : ['bab' => 0, 'alasan' => ''];
        ...
        elseif ($action === 'approve_document') {
            ...
            if ($ok) {
                ...
                if ($newStatus === 'disetujui') {
                    ...
                    if (p2_is_format_doc((string)$chapter['nama_bab'])) $approvedMessage = 'Naskah proposal (Bab 1–3) disetujui Pembimbing 2. Anda siap mengajukan seminar proposal.';
                    elseif (in_array(chapter_number((string)$chapter['nama_bab']), [1, 2, 3], true) && p2_proposal_chapters_done($conn, $guidanceKey) === 3) $approvedMessage .= p2_second_id($conn, $guidanceKey)
                        ? ' Bab 1–3 telah disetujui Pembimbing 1. Unggah naskah proposal (Bab 1–3 dalam satu file) untuk diperiksa format penulisannya oleh Pembimbing 2.'
                        : ' Bab 1–3 telah disetujui Pembimbing 1. Anda dapat memilih Pembimbing 2 untuk pemeriksaan format penulisan, atau langsung mengajukan seminar proposal.';
...
$progressSteps = function (array $guidance) use ($progressMap): array {return thesis_progress_steps($guidance, $progressMap[(int)$guidance['id']] ?? []);};
...
<?php $progressGuidances=array_values(array_filter($guidances,function($guidance){return $guidance['status']!=='ditangguhkan';})); if($progressGuidances): ?><section class="card progress-card" id="progres-skripsi"><div class="section-heading"><span class="eyebrow">PROGRES</span><h2>Progres Skripsi</h2><p>Dari pengajuan judul sampai Bab 5. Setiap bab terbuka setelah bab sebelumnya disetujui.</p></div><?php foreach($progressGuidances as $item): $steps=$progressSteps($item); ?><?php if(count($progressGuidances)>1): ?><p class="progress-title"><?=h($item['judul_skripsi'])?></p><?php endif; ?><ol class="progress-steps"><?php foreach($steps as $index=>$step): ?><li class="step step-<?=h($step['status'])?>"><span class="step-marker" aria-hidden="true"><?=$step['status']==='selesai'?'✓':($index===0?'J':$index)?></span><strong><?=h($step['label'])?></strong><small><?=h(progress_status_label($step['status']))?><?=$step['versi']>1?' · v'.(int)$step['versi']:''?></small></li><?php endforeach; ?></ol><?php endforeach; ?></section><?php endif; ?>
