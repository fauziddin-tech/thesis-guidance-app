...
if ($role === 'dosen') {
    ...
    foreach ($documents as $document) {
        $key = (int)$document['bimbingan_id'] . '|' . mb_strtolower(trim((string)$document['nama_bab']));
        $rank = [(int)$document['versi'], (int)$document['id']];
        if (!isset($latestVersionKey[$key]) || $rank > $latestVersionKey[$key]) $latestVersionKey[$key] = $rank;
    }
    foreach ($documents as &$document) {
        $document['digantikan'] = $latestVersionKey[(int)$document['bimbingan_id'] . '|' . mb_strtolower(trim((string)$document['nama_bab']))] !== [(int)$document['versi'], (int)$document['id']];
