...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($action === 'add_revision' || $action === 'approve_document') && $role === 'dosen') {
        ...
        elseif ($action === 'approve_document') {
            ...
            $revisionMime = '';
            if ($revisionFileProvided && ($revisionFile['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK && function_exists('finfo_open')) {
                $finfo = finfo_open(FILEINFO_MIME_TYPE);
                if ($finfo) {$revisionMime = (string)finfo_file($finfo, $revisionFile['tmp_name']);finfo_close($finfo);}
            }
            $revisionExtension = $revisionFileProvided ? strtolower(pathinfo((string)$revisionFile['name'], PATHINFO_EXTENSION)) : '';
            $revisionAllowedMimes = ['application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/zip', 'application/octet-stream'];
            ...
            elseif ($revisionFileProvided && !$revisionFileSupported) set_flash('danger', 'Fitur lampiran belum diaktifkan pada basis data. Jalankan file migrasi terlebih dahulu.');
            elseif ($revisionFileProvided && ($revisionFile['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) set_flash('danger', 'Lampiran DOCX gagal diterima. Pilih kembali file.');
            elseif ($revisionFileProvided && (($revisionFile['size'] ?? 0) < 1 || $revisionFile['size'] > 10 * 1024 * 1024)) set_flash('danger', 'Ukuran lampiran DOCX maksimal 10 MB.');
            elseif ($revisionFileProvided && ($revisionExtension !== 'docx' || ($revisionMime !== '' && !in_array($revisionMime, $revisionAllowedMimes, true)))) set_flash('danger', 'Lampiran revisi harus berupa file DOCX yang valid.');
            else {
                $revisionRelativePath = null;$revisionAbsolutePath = null;
                if ($revisionFileProvided) {
                    $uploadDirectory = __DIR__ . '/../../storage/uploads';
                    if (!is_dir($uploadDirectory)) mkdir($uploadDirectory, 0750, true);
                    $storedName = 'revision-' . bin2hex(random_bytes(20)) . '.docx';
                ...
                if ($ok) {
                    ...
                    set_flash('success', $revisionFileProvided ? 'Catatan dan lampiran DOCX berhasil dikirim kepada mahasiswa.' : 'Catatan revisi berhasil dikirim kepada mahasiswa.');
