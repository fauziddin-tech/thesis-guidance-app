...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'move_guidance_period' && $role === 'admin' && $academicReady) {
        ...
        if (!$currentGuidance || !in_array($periodId, $periodIds, true) || !in_array($newStatus, ['aktif', 'selesai', 'ditangguhkan'], true)) set_flash('danger', 'Bimbingan, periode, atau status tidak valid.');
        else {
            ...
            if ($ok && $newStatus !== $currentGuidance['status']) {
                ...
                [$statusText, $statusTitle] = $statusMessages[$newStatus];
...
if ($role === 'mahasiswa') {
    ...
    $lecturers = $conn->query("SELECT u.id,u.nama_lengkap,u.email,u.username,u.created_at,COALESCE(SUM(b.status='aktif'),0) AS bimbingan_aktif,COALESCE(SUM(b.status='ditangguhkan'),0) AS bimbingan_ditangguhkan,COALESCE(SUM(b.status='selesai'),0) AS bimbingan_selesai FROM users u LEFT JOIN bimbingan b ON b.dosen_id=u.id WHERE u.role='dosen' GROUP BY u.id,u.nama_lengkap,u.email,u.username,u.created_at ORDER BY u.nama_lengkap")->fetch_all(MYSQLI_ASSOC);
...
<section class="card" id="daftar-dosen"><div class="section-heading"><span class="eyebrow">AKUN DOSEN</span><h2>Daftar Dosen</h2><p><?=count($lecturers)?> dosen terdaftar. Jumlah bimbingan dihitung dari seluruh periode.</p></div><?php if($lecturers): ?><div data-paginate="dosen"><?=paginate_tools('lecturer-search','Cari nama, username, atau email dosen…','dosen')?><div class="table-wrap"><table><thead><tr><th>Dosen</th><th>Username</th><th>Bimbingan Aktif</th><th>Ditangguhkan</th><th>Selesai (Arsip)</th><th>Terdaftar</th></tr></thead><tbody><?php foreach($lecturers as $lecturer): $lecturerAvatar=$getAvatar((int)$lecturer['id']);$lecturerInitials=$getInitials($lecturer['nama_lengkap']); ?><tr data-paginate-item><td><div class="student-identity"><span class="student-avatar-button" aria-hidden="true"><?php if($lecturerAvatar['exists']): ?><img src="<?=h($lecturerAvatar['url'])?>" alt=""><?php else: ?><span><?=h($lecturerInitials)?></span><?php endif; ?></span><div><strong><?=h($lecturer['nama_lengkap'])?></strong><small class="table-subtext"><?=h($lecturer['email'])?></small></div></div></td><td><?=h($lecturer['username'])?></td><td><strong><?=(int)$lecturer['bimbingan_aktif']?></strong></td><td><?=(int)$lecturer['bimbingan_ditangguhkan']?></td><td><?=(int)$lecturer['bimbingan_selesai']?></td><td><?=h(tanggal_id($lecturer['created_at']))?></td></tr><?php endforeach; ?></tbody></table></div><?=paginate_footer('dosen')?></div><?php else: ?><div class="empty-state"><strong>Belum ada dosen.</strong><span>Tambahkan akun dosen melalui formulir Tambah Dosen.</span></div><?php endif; ?></section>
