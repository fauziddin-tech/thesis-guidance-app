...
$documentsByStudent = [];
if ($role === 'dosen') {
    foreach ($documents as $document) $documentsByStudent[(int)$document['mahasiswa_id']][] = $document;
}
...
<section class="card lecturer-student-card" id="mahasiswa-bimbingan"><div class="section-heading"><span class="eyebrow">MAHASISWA</span><h2>Mahasiswa Bimbingan Saya</h2><p>Klik foto untuk mengenali mahasiswa atau pilih View untuk melihat dokumennya.</p></div><?php if($guidances): ?><div class="table-wrap"><table class="student-table"><thead><tr><th>Mahasiswa</th><th>Judul Skripsi</th><th>Status</th><th>View</th></tr></thead><tbody><?php foreach($guidances as $item): $studentId=(int)$item['mahasiswa_id'];$studentAvatar=$getAvatar($studentId);$studentInitials=$getInitials($item['nama_lengkap']);$studentDocuments=$documentsByStudent[$studentId]??[]; ?><tr><td><div class="student-identity"><button class="student-avatar-button" type="button" data-photo-open data-photo-src="<?=h($studentAvatar['url'])?>" data-photo-name="<?=h($item['nama_lengkap'])?>" data-photo-initials="<?=h($studentInitials)?>" aria-label="Lihat foto <?=h($item['nama_lengkap'])?>"><?php if($studentAvatar['exists']): ?><img src="<?=h($studentAvatar['url'])?>" alt=""><?php else: ?><span aria-hidden="true"><?=h($studentInitials)?></span><?php endif; ?></button><div><strong><?=h($item['nama_lengkap'])?></strong><small class="table-subtext"><?=h($item['email'])?></small></div></div></td><td><?=h($item['judul_skripsi'])?></td><td><?=$statusBadge($item['status'])?></td><td><button class="btn btn-secondary btn-small" type="button" data-documents-open="<?=$studentId?>" data-student-name="<?=h($item['nama_lengkap'])?>">View <span class="button-count"><?=count($studentDocuments)?></span></button></td></tr><?php endforeach; ?></tbody></table></div><?php else: ?><div class="empty-state"><strong>Belum ada mahasiswa bimbingan.</strong><span>Data akan tampil setelah mahasiswa ditugaskan kepada Anda.</span></div><?php endif; ?></section>
<?php $renderedStudentTemplates=[];foreach($guidances as $item): $studentId=(int)$item['mahasiswa_id'];if(isset($renderedStudentTemplates[$studentId]))continue;$renderedStudentTemplates[$studentId]=true;$studentDocuments=$documentsByStudent[$studentId]??[]; ?><template id="student-documents-<?=$studentId?>"><?php if($studentDocuments): ?><div class="table-wrap dialog-table-wrap"><table><thead><tr><th>Dokumen</th><th>Unggah</th><th>Status</th><th>Tindakan</th></tr></thead><tbody><?php foreach($studentDocuments as $document): $documentLabel=$document['nama_bab'].' v'.$document['versi'].' — '.$document['nama_lengkap']; ?><tr><td><strong><?=h($document['nama_bab'])?></strong><small class="table-subtext">Versi <?=(int)$document['versi']?></small></td><td><?=date('d/m/Y H:i',strtotime($document['uploaded_at']))?></td><td><?=$statusBadge($document['status'])?></td><td><div class="table-actions"><a class="btn btn-secondary btn-small" href="?action=download&amp;id=<?=(int)$document['id']?>">Unduh</a><button class="btn btn-secondary btn-small" type="button" data-revision-open data-document-id="<?=(int)$document['id']?>" data-document-label="<?=h($documentLabel)?>">Revisi</button><?php if($document['status']!=='disetujui'): ?><form method="post" data-confirm="Setujui dokumen ini?"><?=csrf_field()?><input type="hidden" name="action" value="approve_document"><input type="hidden" name="bab_id" value="<?=(int)$document['id']?>"><button class="btn btn-success btn-small" type="submit">Setujui</button></form><?php endif; ?></div></td></tr><?php endforeach; ?></tbody></table></div><?php else: ?><div class="empty-state"><strong>Belum ada dokumen.</strong><span>Dokumen akan tampil setelah mahasiswa mengirimkan bab untuk review.</span></div><?php endif; ?></template><?php endforeach; ?>
