...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'upload_chapter' && $role === 'mahasiswa') {
        ...
        $uploadState = $guidance ? ($postedChapter === P2_FORMAT_TARGET ? p2_format_upload_state($conn, $guidanceId) : ($postedChapter === PROPOSAL_TARGET ? proposal_upload_state($conn, $guidanceId) : chapter_upload_state($conn, $guidanceId))) : ['bab' => 0, 'alasan' => ''];
        ...
        elseif ($postedChapter === PROPOSAL_TARGET && proposal_checklist_ready($conn) && ($missingTicks = proposal_missing_student_ticks(array_map('strval', (array)($_POST['kelengkapan'] ?? [])))) !== []) set_flash('danger', 'Periksa kembali kelengkapan proposal. Belum dicentang: ' . implode(', ', array_map(function ($key) {return PROPOSAL_COMPONENTS[$key];}, $missingTicks)) . '.');
        elseif (!in_array($extension, $allowedExtensions, true) || $wordFormat === '') set_flash('danger', 'Format file tidak valid. Unggah dokumen Microsoft Word (DOC atau DOCX). Jika file berasal dari aplikasi lain, simpan ulang melalui File > Save As > Word Document.');
            ...
            if (!move_uploaded_file($file['tmp_name'], $absolutePath)) set_flash('danger', 'Dokumen belum dapat disimpan. Periksa izin folder penyimpanan.');
                ...
                $stmt->bind_param('issi', $guidanceId, $chapterName, $relativePath, $version);$saved = $stmt->execute();$newDocumentId = (int)$stmt->insert_id;$stmt->close();
                if (!$saved) {unlink($absolutePath);set_flash('danger', 'Data dokumen belum dapat disimpan. Silakan coba kembali.');}
                    ...
                    $recipientId = $postedChapter === P2_FORMAT_TARGET ? p2_second_id($conn, $guidanceId) : (int)$guidance['dosen_id'];
                    ...
                    $uploadedMessage = 'Dokumen berhasil diunggah dan dosen pembimbing telah diberi notifikasi.';
                    if ($postedChapter === PROPOSAL_TARGET && proposal_checklist_ready($conn)) {
                        $checklist = ['mahasiswa' => array_keys(PROPOSAL_COMPONENTS), 'otomatis' => $wordFormat === 'docx' ? proposal_scan_docx($absolutePath) : []];
                        proposal_checklist_put($conn, $newDocumentId, $checklist);
                        $notFound = proposal_components_not_found($checklist);
                        if ($notFound) $uploadedMessage .= ' Catatan: sistem tidak menemukan judul bagian ' . implode(', ', $notFound) . ' di dalam file. Bila bagian tersebut sudah ada, pastikan judulnya ditulis jelas (mis. DAFTAR PUSTAKA, LAMPIRAN).';
                    }
                    set_flash($uploadedMessage === 'Dokumen berhasil diunggah dan dosen pembimbing telah diberi notifikasi.' ? 'success' : 'warning', $uploadedMessage);
                }
            }
        }
    ...
    if ($action === 'save_proposal_checklist' && $role === 'dosen') {
        $documentId = (int)($_POST['bab_id'] ?? 0);
        $stmt = $conn->prepare('SELECT bs.id,bs.nama_bab,bs.status,bs.bimbingan_id FROM bab_skripsi bs JOIN bimbingan b ON bs.bimbingan_id=b.id WHERE bs.id=? AND ' . p2_match($conn, 'b') . ' LIMIT 1');
        $stmt->bind_param('ii', $documentId, $userId);$stmt->execute();$document = $stmt->get_result()->fetch_assoc();$stmt->close();
        $results = (array)($_POST['hasil'] ?? []);
        if (!$document || !proposal_is_doc((string)$document['nama_bab']) || !proposal_checklist_ready($conn)) set_flash('danger', 'Proposal tidak ditemukan.');
        elseif ($document['status'] !== 'menunggu_review' || !p2_can_review($conn, (int)$document['bimbingan_id'], 'bab', $documentId, $userId)) set_flash('danger', 'Kelengkapan proposal diperiksa oleh Pembimbing 1 selama proposal menunggu review.');
        elseif (array_diff(array_keys(PROPOSAL_COMPONENTS), array_keys(array_filter($results, function ($value) {return in_array($value, ['sesuai', 'perbaikan'], true);})))) set_flash('danger', 'Beri penilaian Sesuai atau Perlu perbaikan untuk setiap komponen.');
        else {
            $checklist = proposal_checklist_get($conn, $documentId);
            $checklist['dosen'] = ['hasil' => array_intersect_key($results, PROPOSAL_COMPONENTS), 'oleh' => $userId, 'waktu' => date('c')];
            proposal_checklist_put($conn, $documentId, $checklist);
            $needsFix = proposal_components_with($checklist, 'perbaikan');
            set_flash('success', $needsFix ? 'Pemeriksaan disimpan. Klik Revisi untuk mengirim catatan; komponen yang perlu diperbaiki (' . implode(', ', $needsFix) . ') otomatis dicantumkan.' : 'Pemeriksaan disimpan. Semua komponen sesuai; proposal dapat disetujui.');
        }
    ...
    if (($action === 'add_revision' || $action === 'approve_document') && $role === 'dosen') {
        ...
        elseif (!p2_can_review($conn, (int)$chapter['bimbingan_id'], 'bab', (int)$chapter['id'], $userId)) set_flash('danger', (p2_is_format_doc((string)$chapter['nama_bab']) ? 'Naskah proposal diperiksa oleh Pembimbing 2.' : 'Bab skripsi direview oleh Pembimbing 1. Pembimbing 2 memeriksa naskah proposal (Bab 1–3) setelah Bab 1–3 disetujui.'));
        elseif ($action === 'approve_document' && $myDecision === 'disetujui') set_flash('danger', 'Anda sudah menyetujui dokumen ini.');
        elseif ($action === 'approve_document' && proposal_is_doc((string)$chapter['nama_bab']) && proposal_checklist_ready($conn) && !proposal_checklist_complete(proposal_checklist_get($conn, (int)$chapter['id']))) set_flash('danger', 'Periksa kelengkapan proposal terlebih dahulu. Proposal dapat disetujui bila semua komponen bertanda Sesuai; bila ada yang perlu diperbaiki, gunakan Revisi.');
        elseif ($action === 'approve_document') {
            ...
            if ($ok) {
                ...
                if ($newStatus === 'disetujui') {
                    ...
                    $guidanceKey = (int)$chapter['bimbingan_id'];
                    unset($GLOBALS['p2LatestCache'][$guidanceKey]);
                    $approvedMessage = $chapter['nama_bab'] . ' telah disetujui dosen pembimbing.';
                    if (p2_is_format_doc((string)$chapter['nama_bab'])) $approvedMessage = 'Naskah Bab 1–3 disetujui Pembimbing 2 (format penulisan). Selanjutnya unggah proposal penelitian lengkap: Bab 1–3 beserta instrumen penelitian.';
                    elseif (proposal_is_doc((string)$chapter['nama_bab'])) $approvedMessage = 'Proposal penelitian disetujui Pembimbing 1. Anda siap mengikuti seminar proposal. Bab 4 terbuka setelah seminar proposal ditandai selesai.';
                    elseif (in_array(chapter_number((string)$chapter['nama_bab']), [1, 2, 3], true) && p2_proposal_chapters_done($conn, $guidanceKey) === 3) $approvedMessage .= p2_second_id($conn, $guidanceKey)
                        ? ' Bab 1–3 telah disetujui Pembimbing 1. Unggah naskah Bab 1–3 dalam satu file untuk diperiksa format penulisannya oleh Pembimbing 2, lalu unggah proposal penelitian.'
                        : ' Bab 1–3 telah disetujui Pembimbing 1. Selanjutnya unggah proposal penelitian lengkap (Bab 1–3 beserta instrumen penelitian), atau pilih Pembimbing 2 terlebih dahulu bila diwajibkan program studi.';
                    notify_user($conn, $recipientId, 'persetujuan', $approvedMessage, $link, 'Dokumen disetujui: ' . $chapter['nama_bab'], 'Dokumen Anda disetujui', 'Lihat Dokumen');
            ...
            if ($chapter && proposal_is_doc((string)$chapter['nama_bab']) && proposal_checklist_ready($conn) && ($needsFix = proposal_components_with(proposal_checklist_get($conn, (int)$chapter['id']), 'perbaikan'))) $comment = 'Kelengkapan/tata tulis yang perlu diperbaiki: ' . implode(', ', $needsFix) . ".\n\n" . $comment;
    ...
    if (($action === 'approve_title' || $action === 'revise_title') && $role === 'dosen') {
        ...
        elseif (approval_is_pending($titleGuidance)) set_flash('danger', 'Terima pendaftaran mahasiswa ini terlebih dahulu sebelum mereview judulnya.');
        elseif (!p2_can_review($conn, $guidanceId, 'judul', $guidanceId, $userId)) set_flash('danger', 'Pengajuan judul direview oleh Pembimbing 1.');
    ...
    if ($action === 'choose_second_supervisor' && $role === 'mahasiswa') {
        try {
            ...
            set_flash('success', 'Pembimbing 2 berhasil disimpan dan telah diberi notifikasi. Unggah naskah proposal (Bab 1–3 dalam satu file) pada bagian Unggah Bab.');
        } catch (Throwable $error) {
    ...
    if ($action === 'request_guidance' && $role === 'mahasiswa') {
        ...
        elseif (!valid_thesis_title($title)) set_flash('danger', 'Judul skripsi wajib diisi, 10–255 karakter.');
            ...
            set_flash($ok ? 'success' : 'danger', $ok ? 'Pendaftaran bimbingan terkirim. Bimbingan aktif setelah diterima dosen pembimbing.' : 'Bimbingan belum dapat dibuat.');
        }
        $redirectDashboard('bimbingan-saya');
    }
    ...
    if ($action === 'accept_registration' && $role === 'dosen') {
        try {
            approval_accept($conn, $userId, (int)($_POST['bimbingan_id'] ?? 0));
            set_flash('success', 'Pendaftaran diterima. Mahasiswa telah diberi notifikasi, dan pengajuan judulnya kini dapat Anda review.');
        } catch (Throwable $error) {
            set_flash('danger', $error instanceof RuntimeException ? $error->getMessage() : 'Pendaftaran belum dapat diterima.');
        }
        $redirectDashboard('pendaftaran-baru');
    }

    if (($action === 'reject_registration' && $role === 'dosen') || ($action === 'delete_guidance' && in_array($role, ['dosen', 'admin'], true))) {
        $reason = trim(preg_replace('/\s+/', ' ', (string)($_POST['alasan'] ?? '')));
        if (mb_strlen($reason) > 500) set_flash('danger', 'Alasan maksimal 500 karakter.');
        else {
            try {
                $deleted = guidance_delete($conn, (int)($_POST['bimbingan_id'] ?? 0), ['id' => $userId, 'role' => $role, 'nama_lengkap' => (string)$user['nama_lengkap']], $reason, $action === 'reject_registration');
                set_flash('success', ($action === 'reject_registration' ? 'Pendaftaran ' . $deleted['mahasiswa'] . ' ditolak' : 'Bimbingan ' . $deleted['mahasiswa'] . ' dihapus') . ($deleted['akun_dihapus'] ? ' beserta akun mahasiswanya.' : '. Akun mahasiswa tetap ada karena masih memiliki bimbingan lain.'));
            } catch (Throwable $error) {
                set_flash('danger', $error instanceof RuntimeException ? $error->getMessage() : 'Data belum dapat dihapus.');
            }
        }
        $redirectDashboard($action === 'reject_registration' ? 'pendaftaran-baru' : ($role === 'admin' ? 'daftar-bimbingan' : 'mahasiswa-bimbingan'));
    }

    if ($action === 'mark_seminar_done' && $role === 'dosen') {
        try {
            proposal_mark_seminar($conn, ['id' => $userId, 'role' => $role], (int)($_POST['bimbingan_id'] ?? 0), trim((string)($_POST['tanggal_seminar'] ?? '')));
            set_flash('success', 'Seminar proposal ditandai selesai. Bab 4 kini terbuka bagi mahasiswa, dan mahasiswa telah diberi notifikasi.');
        } catch (Throwable $error) {
            set_flash('danger', $error instanceof RuntimeException ? $error->getMessage() : 'Seminar proposal belum dapat ditandai.');
        }
        $redirectDashboard('mahasiswa-bimbingan');
    }

    if ($action === 'unmark_seminar' && $role === 'dosen') {
        try {
            proposal_unmark_seminar($conn, $userId, (int)($_POST['bimbingan_id'] ?? 0));
            set_flash('success', 'Tanda seminar proposal dibatalkan.');
        } catch (Throwable $error) {
            set_flash('danger', $error instanceof RuntimeException ? $error->getMessage() : 'Tanda seminar belum dapat dibatalkan.');
        }
        $redirectDashboard('mahasiswa-bimbingan');
    }

    if ($action === 'release_second_supervisor' && $role === 'admin') {
        try {
            p2_release($conn, (int)($_POST['bimbingan_id'] ?? 0));
            set_flash('success', 'Pembimbing 2 dilepas. Mahasiswa dan dosen terkait telah diberi notifikasi.');
        } catch (Throwable $error) {
            set_flash('danger', $error instanceof RuntimeException ? $error->getMessage() : 'Pembimbing 2 belum dapat dilepas.');
        }
        $redirectDashboard('daftar-bimbingan');
    }

    if ($action === 'run_backup' && $role === 'admin') {
        $lastBackup = backup_status();
        if ($lastBackup && !empty($lastBackup['ok']) && time() - (int)strtotime((string)$lastBackup['waktu']) < 120) {
            set_flash('warning', 'Backup baru saja dibuat. Tunggu sekitar dua menit sebelum membuat backup lagi.');
        } else {
            @set_time_limit(300);
            try {
                $backupResult = backup_run($conn, (string)$dbName, 'admin:' . $userId);
                set_flash('success', 'Backup berhasil dibuat: ' . $backupResult['file'] . ' (' . number_format($backupResult['ukuran'] / 1024, 1, ',', '.') . ' KB, ' . $backupResult['tabel'] . ' tabel).');
            } catch (Throwable $error) {
                set_flash('danger', 'Backup gagal: ' . $error->getMessage());
            }
        }
        $redirectDashboard('backup-database');
    }
    ...
    if ($action === 'record_meeting' && $role === 'mahasiswa') {
        $draft = ['bimbingan_id' => (int)($_POST['bimbingan_id'] ?? 0), 'dosen_id' => (int)($_POST['dosen_id'] ?? 0), 'tanggal' => trim((string)($_POST['tanggal'] ?? '')), 'metode' => (string)($_POST['metode'] ?? ''), 'topik' => trim(preg_replace('/\s+/', ' ', (string)($_POST['topik'] ?? ''))), 'catatan' => trim((string)($_POST['catatan'] ?? ''))];
        try {
            if (approval_guidance_pending($conn, $draft['bimbingan_id'])) throw new RuntimeException('Pertemuan dapat dicatat setelah pendaftaran Anda diterima dosen pembimbing.');
            meeting_record($conn, $user, $draft['bimbingan_id'], $draft['dosen_id'], $draft['tanggal'], $draft['metode'], $draft['topik'], $draft['catatan']);
            unset($_SESSION['meeting_draft']);
            set_flash('success', 'Log pertemuan tersimpan dan menunggu konfirmasi dosen pembimbing.');
        } catch (Throwable $error) {
            $_SESSION['meeting_draft'] = $draft;
            set_flash('danger', $error instanceof RuntimeException ? $error->getMessage() : 'Log pertemuan belum dapat disimpan.');
        }
        $redirectDashboard('log-pertemuan');
    }

    if ($action === 'cancel_meeting' && $role === 'mahasiswa') {
        try {
            meeting_cancel($conn, $userId, (int)($_POST['log_id'] ?? 0));
            set_flash('success', 'Log pertemuan dibatalkan.');
        } catch (Throwable $error) {
            set_flash('danger', $error instanceof RuntimeException ? $error->getMessage() : 'Log pertemuan belum dapat dibatalkan.');
        }
        $redirectDashboard('log-pertemuan');
    }

    if (($action === 'confirm_meeting' || $action === 'reject_meeting') && $role === 'dosen') {
        try {
            meeting_decide($conn, $userId, (int)($_POST['log_id'] ?? 0), $action === 'confirm_meeting', trim((string)($_POST['alasan'] ?? '')));
            set_flash('success', $action === 'confirm_meeting' ? 'Pertemuan dikonfirmasi dan mahasiswa telah diberi notifikasi.' : 'Penolakan log pertemuan telah dikirim kepada mahasiswa.');
        } catch (Throwable $error) {
            set_flash('danger', $error instanceof RuntimeException ? $error->getMessage() : 'Tanggapan belum dapat disimpan.');
        }
        $redirectDashboard('konfirmasi-pertemuan');
    }
...
// Log pertemuan bimbingan (aktif setelah migrasi 20260927_add_meeting_log.sql).
$meetingReady = meeting_ready($conn);
$meetingCounts = $meetingReady ? meeting_confirmed_counts($conn, array_column($guidances, 'id')) : [];
$meetingLogs = ($meetingReady && $role === 'mahasiswa') ? meeting_logs_by_guidance($conn, array_column($guidances, 'id')) : [];
$pendingMeetings = ($meetingReady && $role === 'dosen') ? meeting_pending_for_lecturer($conn, $userId) : [];
$meetingDraft = $_SESSION['meeting_draft'] ?? [];
// Penerimaan pendaftaran mahasiswa dan bimbingan yang boleh dihapus (tanpa dokumen/pertemuan terkonfirmasi).
$pendingRegistrations = $role === 'dosen' ? approval_pending_for_lecturer($conn, $userId) : [];
$deletableGuidances = in_array($role, ['dosen', 'admin'], true) ? guidance_deletable_ids($conn, array_column($guidances, 'id')) : [];
unset($_SESSION['meeting_draft']);
...
$progressMap = chapter_progress_map($conn, array_column($guidances, 'id'));
$progressSteps = function (array $guidance) use ($progressMap, $conn): array {return thesis_progress_steps($guidance, $progressMap[(int)$guidance['id']] ?? [], proposal_ready($conn) ? proposal_stage($conn, (int)$guidance['id']) : null);};
...
<?php include __DIR__.'/backup-admin.php'; ?>
