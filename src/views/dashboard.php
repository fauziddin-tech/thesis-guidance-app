...
if ($role === 'dosen' && ($periodFilter || $runningOnly)) {
...
$renderFilter = function (string $anchor, bool $withProdi) use ($periods, $periodFilter, $prodiList, $prodiFilter, $runningOnly): string {
    $html = '<form method="get" class="period-filter" action="#' . h($anchor) . '"><input type="hidden" name="page" value="dashboard">';
    $html .= '<label>Periode <select name="periode" data-auto-submit><option value="semua">Semua periode</option><option value="berjalan"' . ($runningOnly ? ' selected' : '') . '>Masih berjalan (semua periode)</option>';
