...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $rateKeyAccount=rl_key($identity);$rateKeyIp=rl_key(client_ip());
    if(!verify_csrf()){$loginError='Sesi formulir telah berakhir. Muat ulang halaman dan coba kembali.';}
    elseif(rl_count($conn,'login',$rateKeyAccount,900)>=5||rl_count($conn,'login_ip',$rateKeyIp,900)>=20){$loginError='Terlalu banyak percobaan masuk yang gagal. Coba lagi dalam 15 menit, atau gunakan Lupa password.';}
    else{
