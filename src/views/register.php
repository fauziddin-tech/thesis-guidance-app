...
if($_SERVER['REQUEST_METHOD']==='POST'){
    $username=trim($_POST['username']??'');$email=trim($_POST['email']??'');$nama=trim(preg_replace('/\s+/u',' ',(string)($_POST['nama_lengkap']??'')));$password=$_POST['password']??'';$confirm=$_POST['confirm_password']??'';$role='mahasiswa';$telp=trim($_POST['no_telp']??'');
    ...
    elseif(trim((string)($_POST['website']??''))!==''){$registerMessage='Pendaftaran belum berhasil. Silakan coba kembali.';}
    elseif(rl_count($conn,'register_ip',rl_key(client_ip()),3600)>=10){$registerMessage='Terlalu banyak pendaftaran dari jaringan ini. Coba lagi dalam satu jam atau hubungi administrator.';}
    ...
    elseif($lecturerChoices&&!valid_thesis_title($title)){$registerMessage='Judul skripsi wajib diisi, 10–255 karakter.';}
        ...
        elseif($nimExists){$registerMessage='NIM sudah terdaftar. Hubungi administrator jika NIM tersebut milik Anda.';}
            ...
            if($ok)rl_hit($conn,'register_ip',rl_key(client_ip()));
            $mailSent=false;
            if($ok){
