...
if($action==='create_bimbingan'){
 ...
 $stmt=$conn->prepare('INSERT INTO bimbingan(mahasiswa_id,dosen_id,judul_skripsi,deskripsi) VALUES(?,?,?,?)');$stmt->bind_param('iiss',$uid,$dosen_id,$judul,$deskripsi);$ok=$stmt->execute();$stmt->close();flash($ok?'success':'danger',$ok?'Bimbingan berhasil dibuat.':'Bimbingan gagal dibuat.');redirect('?page=dashboard');
}
...
if($action==='upload_bab'){
 ...
 $stmt=$conn->prepare("SELECT id FROM bimbingan WHERE id=? AND mahasiswa_id=? AND status='aktif'");$stmt->bind_param('ii',$bid,$uid);$stmt->execute();$valid=$stmt->get_result()->num_rows;$stmt->close();if(!$valid){flash('danger','Bimbingan tidak valid.');redirect('?page=dashboard');}
 ...
 $dir=__DIR__.'/uploads/bab';if(!is_dir($dir))mkdir($dir,0750,true);$name=bin2hex(random_bytes(16)).'.'.$allowed[$mime];$path=$dir.'/'.$name;if(!move_uploaded_file($file['tmp_name'],$path)){flash('danger','File gagal disimpan.');redirect('?page=dashboard');}
 $stmt=$conn->prepare('SELECT COALESCE(MAX(versi),0)+1 v FROM bab_skripsi WHERE bimbingan_id=? AND nama_bab=?');$stmt->bind_param('is',$bid,$nama);$stmt->execute();$versi=(int)$stmt->get_result()->fetch_assoc()['v'];$stmt->close();$relative='uploads/bab/'.$name;$status='menunggu_review';$stmt=$conn->prepare('INSERT INTO bab_skripsi(bimbingan_id,nama_bab,file_path,versi,status) VALUES(?,?,?,?,?)');$stmt->bind_param('issis',$bid,$nama,$relative,$versi,$status);$ok=$stmt->execute();$stmt->close();if(!$ok)@unlink($path);flash($ok?'success':'danger',$ok?'Bab berhasil diunggah dan menunggu review dosen.':'Data file gagal disimpan.');redirect('?page=dashboard');
}
...
if($action==='add_revision'){
 ...
 $stmt=$conn->prepare('SELECT bs.id FROM bab_skripsi bs JOIN bimbingan b ON b.id=bs.bimbingan_id WHERE bs.id=? AND b.dosen_id=?');$stmt->bind_param('ii',$bab,$uid);$stmt->execute();$valid=$stmt->get_result()->num_rows;$stmt->close();if(!$valid){flash('danger','Bab tidak berada dalam bimbingan Anda.');redirect('?page=dashboard');}
 $stmt=$conn->prepare('INSERT INTO revisi(bab_id,dosen_id,komentar,tipe_revisi) VALUES(?,?,?,?)');$stmt->bind_param('iiss',$bab,$uid,$komentar,$tipe);$ok=$stmt->execute();$stmt->close();if($ok){$stmt=$conn->prepare("UPDATE bab_skripsi SET status='direvisi' WHERE id=?");$stmt->bind_param('i',$bab);$stmt->execute();$stmt->close();}flash($ok?'success':'danger',$ok?'Revisi berhasil dikirim.':'Revisi gagal dikirim.');redirect('?page=dashboard');
}

$flash=take_flash();if($page==='dashboard')require_login();
?>
