...
if($action==='upload_bab'){
 ...
 $stmt=$conn->prepare('SELECT COALESCE(MAX(versi),0)+1 v FROM bab_skripsi WHERE bimbingan_id=? AND nama_bab=?');$stmt->bind_param('is',$bid,$nama);$stmt->execute();$versi=(int)$stmt->get_result()->fetch_assoc()['v'];$stmt->close();$relative='uploads/bab/'.$name;$stmt=$conn->prepare('INSERT INTO bab_skripsi(bimbingan_id,nama_bab,file_path,versi,status) VALUES(?,?,?,?,?)');$status='menunggu_review';$stmt->bind_param('issis',$bid,$nama,$relative,$versi,$status);$ok=$stmt->execute();$stmt->close();if(!$ok)@unlink($path);flash($ok?'success':'danger',$ok?'Bab berhasil diunggah dan menunggu review dosen.':'Data file gagal disimpan.');redirect('?page=dashboard');
}
...
$flash=take_flash();
if(in_array($page,['dashboard','profile','change-password','bimbingan-detail','admin-dashboard'],true))require_login();
?>
